...
const Navbar = () => {
    ...
    return (
        <div className="flex justify-between items-center py-4">
            <Link to="/" className="flex  font-bold   items-center  gap-2 "><img src="/main1.png" alt="Logo" className="size-8" /><p className="racking-wide text-[20px] sm:text-[30px]">Genarata</p></Link>
            {
                context.user ?
                    <div className="flex items-center gap-2 sm:gap-3">
                        <button className="flex gap-2 items-center bg-blue-100 px-4 sm:px-6 py-1.5 sm:py-3 rounded-full drop-shadow hover:scale-105 transition-all duration-700"><img src={assets.credit_star} alt="credit_star" />
                            <p className="text-xs sm:text-sm text-gray-600 font-medium" onClick={() => navigate("/buy")}>Credits left: {context.credit}</p>
                        </button>
                        <p className="text-gray-600 max-sm:hidden pl-4">Hi ,{context.user}</p>
                        <div className="relative group">
                            ...
                            <div className="absolute hidden group-hover:block top-0 right-0 z-10 rounded text-black pt-12">
                                <ul className="list-none m-0 py-2 bg-white rounded-md text-sm border">
                                    <li className=" pr-10 px-2 py-1 cursor-pointer drop-shadow" onClick={context.logout}> Logout</li>
                                </ul>
                            </div>
                        </div>
                    </div>
