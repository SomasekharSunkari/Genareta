        ...
        <motion.form initial={{ opacity: 0, y: 100 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: .3 }} onSubmit={onsubmitHndler} className="flex flex-col  min-h-[85vh]" >
            ...
            {!isImageLoading &&
                <div className="flex mx-auto  w-full max-w-xl bg-neutral-500 text-white text-sm mt-10 p-0.5 rounded-full ">
                    <input onChange={(e) => setInput(e.target.value)} value={input} type="text" placeholder="Describe what you want to genareate " className="flex-1 bg-transparent outline-none ml-8  mr-2 max-sm:w-20" />
            ...
            {isImageLoading &&
                <div className="flex flex-wrap  mt-10 rounded-full  text-sm p-0.5 gap-2 justify-center ">
                    <p className="bg-transparent border-2  border-zinx-900 px-8 py-3 rounded-full  cursor-pointer text-black hover:scale-110 hover:bg-black hover:text-white transition-all duration-300" onClick={handleClick}>Generate Another </p>
